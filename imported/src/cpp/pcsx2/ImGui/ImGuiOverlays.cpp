...
__ri void ImGuiManager::DrawPerformanceOverlay(float& position_y, float scale, float margin, float spacing)
{
	if (GSConfig.OsdPerformancePos == OsdOverlayPos::None)
		return; // disabled by setting (finally!)
...
#define IPSX2_VERSION_STR "dev"
#endif
			// [V15] IPSX2_VERSION_STR is already a quoted string literal from CMake -D; no STRINGIFY needed.
			text.append_format("{}iPSX2 v{}", first ? "" : " | ", IPSX2_VERSION_STR);
			first = false;
		}
