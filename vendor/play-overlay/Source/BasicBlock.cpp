...
#endif
	// AYS2: GetCode() (not GetWritableCode()) for the jump target — under
	// iOS 26 TXM dual-mapping this must stay the RX address, since it's
	// what this block's own compiled code will actually jump/call to at
	// runtime. GetWritableCode() must only be used for the WRITE below, and
	// must be captured AFTER BeginModify() (seam) — on non-dual-mapped
	// platforms GetWritableCode()==GetCode() always, so this reordering is
	// a no-op there; on dual-mapped iOS it's what makes the write land on
	// the writable alias instead of the execute-only page.
	auto patchValue = reinterpret_cast<uintptr_t>(otherBlock->m_function.GetCode());
	m_function.BeginModify();
	auto code = reinterpret_cast<uint8*>(m_function.GetWritableCode());
...
void CBasicBlock::UnlinkBlock(LINK_SLOT linkSlot)
{
	...
	// AYS2: same GetWritableCode()-after-BeginModify() reordering as
	// LinkBlock above — see the comment there.
	m_function.BeginModify();
	auto code = reinterpret_cast<uint8*>(m_function.GetWritableCode());
	*reinterpret_cast<uintptr_t*>(code + m_linkBlockTrampolineOffset[linkSlot]) = patchValue;
	m_function.EndModify();
