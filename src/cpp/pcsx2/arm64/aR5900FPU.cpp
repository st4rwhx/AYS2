...
#define ARMSX2_IOS_FPU_PERF_PROBE 1
#else
#define ARMSX2_IOS_FPU_PERF_PROBE 0
#endif

namespace a64 = vixl::aarch64;

// Compile-time-only tester marker: confirms that full-clamp (eeClampMode=3 /
// fpuFullMode — NFS Carbon and friends) FPU ops are being compiled by the native
// ARM64 generators instead of falling back to the interpreter. Logs the first few
// compiles and then every 1024th; emitted blocks themselves carry zero overhead.
#if ARMSX2_IOS_FPU_PERF_PROBE
static u64 s_fpu_full_compiled = 0;
static void fpuFullMarkCompiled(const char* what)
{
	s_fpu_full_compiled++;
	if (s_fpu_full_compiled <= 4 || (s_fpu_full_compiled & 0x3FF) == 0)
	{
		std::fprintf(stderr, "@@FPU_FULL_FASTPATH@@ compiled=%llu last=%s\n",
			static_cast<unsigned long long>(s_fpu_full_compiled), what);
		std::fflush(stderr);
	}
}
#define FPU_FULL_MARK(what) fpuFullMarkCompiled(what)
#else
#define FPU_FULL_MARK(what) ((void)0)
#endif
...
static void emitFpuBinary(FpuBinOp op, u32 dstByteOffset, u32 fs, u32 ft)
{
	...
	{
		FPU_FULL_MARK(op == FpuBinOp::Mul ? "mul" : (op == FpuBinOp::Add ? "add" : "sub"));
		if (op == FpuBinOp::Mul)
		{
...
static void emitFpuMulAcc(bool subtract, bool toAcc, u32 fd, u32 fs, u32 ft)
{
	if (EmuConfig.Cpu.Recompiler.fpuFullMode)
	{
		FPU_FULL_MARK(toAcc ? (subtract ? "msuba" : "madda") : (subtract ? "msub" : "madd"));
