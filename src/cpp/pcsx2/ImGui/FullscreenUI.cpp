	...
	*menu_size = ImVec2(io.DisplaySize.x, io.DisplaySize.y - heading_size.y - LayoutScale(LAYOUT_FOOTER_HEIGHT));
	...
	if (BeginFullscreenWindow(ImVec2(0.0f, 0.0f), heading_size, "landing_heading", UIPrimaryColor))
	{
		...
		// draw branding
		{
			...
			ImGuiFullscreen::AddTextWithShadow(dl, heading_font, branding_pos, ImGui::GetColorU32(ImGuiCol_Text), "AYS2"); // AYS2: fullscreen heading brand (seam)
		}
...
void FullscreenUI::DrawExitWindow()
{
	...
	if (BeginHorizontalMenu("exit_window", menu_pos, menu_size, (Host::InNoGUIMode()) ? 2 : 3))
	{
		...
		if (HorizontalMenuSvgItem("fullscreenui/exit.svg", FSUI_CSTR("Exit AYS2"),
...
void FullscreenUI::DrawAboutWindow()
{
	...
	ImGui::SetNextWindowPos(ImGui::GetIO().DisplaySize * 0.5f, ImGuiCond_Always, ImVec2(0.5f, 0.5f));
	ImGui::OpenPopup(FSUI_CSTR("About AYS2"));
	...
	if (ImGui::BeginPopupModal(FSUI_CSTR("About AYS2"), &s_about_window_open, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize))
	{
